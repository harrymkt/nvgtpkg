...
def list_installed_packages(args):
	...
	for package in l:
		...
		json_path = os.path.join(fullp, "package.json")
		if os.path.exists(json_path):
			with open(json_path, "r") as f:
				pg = json.load(f)
		else:
			pg = dict()
		print(f"{pg.get('name', package)} by {pg.get('author', {}).get('name', 'Unknown')}")
def show_package(args):
	...
	for package in l:
		if package.lower() == args.name.lower():
			...
			name = package
	if haspack:
		json_path = os.path.join(packstore, name, "package.json")
		if os.path.exists(json_path):
			with open(json_path, "r") as f:
				pg = json.load(f)
		else:
...
def package_info(name, pg):
	...
	if not pg.get('name', name) == name:
		...
		print(name)
	print("Author:")
	print(pg.get("author", {}).get("name", "Unknown"))
	...
	try:
		...
		for item in contents:
			if item["type"] == "file":
				...
				matching_files.extend(list_files_in_repo(owner, repo, item["path"], pattern, recursive))
